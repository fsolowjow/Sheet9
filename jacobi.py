...
#wikipedia formula for a and b in T matrix, since def in lecture does not work:
def coeff_a_matrix(n , alpha , beta):
	if(n==0):	return (beta-alpha)/(2+alpha+beta)
	else:		return (beta-alpha)*(alpha+beta)/((2*n+alpha+beta)*(2*n+2+alpha+beta))

def coeff_b_matrix(n , alpha , beta):
	return np.sqrt((4*n*(n+alpha)*(n+beta)*(n+alpha+beta))/((2*n-1+alpha+beta)*(2*n+alpha+beta)*(2*n+alpha+beta)*(2*n+1+alpha+beta)))

#Lecture formula for recursion coeffs.
def coeff_a_rec(n , alpha , beta):
	return (2/(2*n + alpha + beta  ) * np.sqrt((n*(n+alpha+beta)*(n+alpha)*(n+beta)) / ((2*n+alpha+beta-1)*(2*n+alpha+beta+1))))
	
def coeff_b_rec(n , alpha , beta):
...
def jacobiP(x, alpha, beta, n):
	...
	if(n>1):	return ( (x*jacobiP(x,alpha,beta, n-1) - coeff_a_rec(n-1,alpha,beta) * jacobiP(x,alpha,beta, n-2) - coeff_b_rec(n-1,alpha,beta) * jacobiP(x,alpha,beta, n-1)) / coeff_a_rec(n,alpha,beta))
...
def JacobiQuad( n, alpha , beta):
	T=np.zeros((n+1,n+1))
	for i in range(n+1):
		T[i,i] = coeff_a_matrix(i , alpha , beta)
		if(i < n): T[i,i+1] = coeff_b_matrix( i+1 , alpha , beta)
		if(i > 0): T[i,i-1] = coeff_b_matrix( i , alpha , beta)
	#quadrature points  = eigenvalues
	#quadrature weights = eigenvector_j[0]^2 * integral w(x), with w(x) = (1-x)^alpha (1+x)^beta.
	eigenvalue , eigenvector = LA.eig(T)
	
	quadrature_points = eigenvalue
	weight_integral = quad( integrand , -1 , 1 )[0]
	quadrature_weights = eigenvector[0,:] * eigenvector[0,:] * weight_integral
	return quadrature_points, quadrature_weights

#d)
#Does not find any roots. Doing it like in c) would have been smarter
